...
def _load_chunk_texts_for_ids(path: str, chunk_ids: Set[str]) -> Dict[str, str]:
    """Stream chunks; load text only for given chunk_ids (low memory)."""
    out: Dict[str, str] = {}
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
...
def _retrieve(
    ...
    chunk_texts = _load_chunk_texts_for_ids(CHUNKS_JSONL, needed_cids)
    n_found = sum(1 for cid in needed_cids if chunk_texts.get(cid))
    if n_found == 0 and needed_cids:
        print(
            "Warning: No chunk texts found for retrieved IDs. "
    ...
    elif n_found < len(needed_cids):
        print(f"Warning: Found text for {n_found}/{len(needed_cids)} chunks (some IDs missing in chunks.jsonl).", flush=True)
...
def main() -> None:
    ...
    # 터미널 출력 + MD 파일 자동 저장을 위해 전체 결과를 마크다운으로 조립
    lines: list[str] = []
    ...
    for pid in sorted(parent_meta.keys()):
        ...
        if not parts:
            ...
            print(line)
            lines.append(f"- **{pid}** (doc_id={doc_id}, title={title or 'N/A'}) [no text]\n\n")
            continue
        text = "\n\n".join(parts).strip()
        text = _truncate_tokens(text)
        if title:
    ...
    # 2: generate.py 실행 후 결과를 MD 파일로 자동 저장
